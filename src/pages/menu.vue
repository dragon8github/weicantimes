<template>
    ...
    <div class="menu">
            ...
            <div class="mui-control-content" v-for="(item,index) in items">
                <ul class="mui-table-view">
                	<div  :id="'content' + index" >
	                	<div  class="category">
	                		<div class="category-title"><strong class="category-name">{{ item.key }}</strong> <span class="category-description"></span></div>
	                	</div>
	                    ...
	                    <li class="mui-table-view-cell mui-media" v-for="(item2,index2) in item.value">
	                        <a href="javascript:;">
	                            ...
	                            <div class="mui-media-body">
	                                <h4>{{ item2.name }}</h4>
	                                <p class="mui-ellipsis"><span class="mui-ellipsis-left">Monthly sales 
					 {{ item2.sales }}</span><span class="mui-ellipsis-right">Applause Rate {{ item2.rate }}</span></p>
	                                <p class="money"><span class="money-left">$</span><span class="money-right">{{ item2.money }}</span>
        ...
        <div class="cartfooter" style="z-index: 11;"><span class="carticon" attr-quantity="1"></span>
            <div style="padding-top:13px;">
                <p class="carttotal">¥8</p>
                <p class="cartdelivery">再买¥12减配送费¥4.00</p>
            </div><a href="javascript:" class="submitbutton">去结算</a>
        </div>
    </div>
...
export default {
  ...
  created () {
	this.setDate();
	// 每次滚动都判断数组是否为空，如果为空，说明全部都已经淡出完成
    $("#segmentedControlContents").scroll(function () {
        let top = $(".mui-col-xs-9").scrollTop();
        console.log(top);
    });
...
.mui-col-xs-3,.mui-col-xs-9{padding-bottom:100px}
